...
if (isset($_POST['btnupdate'])) {
    ...
    if (!empty($f_name)) {
        ...
        if (in_array($f_extension, ['jpg', 'jpeg', 'png', 'gif'])) {
            if ($f_size <= 1000000) {
                if (move_uploaded_file($f_tmp, $store)) {
                    $update = $pdo->prepare("UPDATE about SET image = :logo, description = :description WHERE id = :id");
                    ...
                    if ($update->execute()) {
                        echo '<script>swal({ title: "Success!", text: "About updated successfully.", icon: "success", button: "Ok" });</script>';
                    } else {
                        echo '<script>swal({ title: "Error!", text: "Failed to update about details.", icon: "error", button: "Ok" });</script>';
                    }
                }
            } else {
        ...
        $update = $pdo->prepare("UPDATE about
 SET  description = :description WHERE id = :id");
        ...
        if ($update->execute()) {
            echo '<script>swal({ title: "Success!", text: "About Details updated successfully.", icon: "success", button: "Ok" });</script>';
        } else {
            echo '<script>swal({ title: "Error!", text: "Failed to update about details.", icon: "error", button: "Ok" });</script>';
        }
    }
            ...
            <form action="" method="post" enctype="multipart/form-data">
                <div class="box-body">
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            <label>Image</label>
                            <img src="aboutimages/<?php echo htmlspecialchars($logo_db); ?>" class="img-responsive" width="50" height="50" alt="Blog Image" />
